        ...
        if (pdfText && pdfText.trim().length > 0) {
          ...
          try {
            ...
            if (cloudErrorMsg.includes('API key') || cloudErrorMsg.includes('not configured')) {
              console.error("⚠️ PDF_CO_API_KEY not set in Vercel environment variables");
              throw new Error("PDF conversion service is not configured. Please set PDF_CO_API_KEY environment variable in Vercel. Go to Vercel Dashboard → Settings → Environment Variables → Add PDF_CO_API_KEY");
            }
